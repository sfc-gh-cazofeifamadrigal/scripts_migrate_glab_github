...
$configPath = Join-Path -Path $PSScriptRoot -ChildPath "cleaned_snowflakecorp_SE_sit.json"
$workingDir = "$PSScriptRoot/reposSIT"
...
function Set-BranchProtection {
    ...
    Write-Output ("Setting branch protection for $branchName in {0}" -f $repoNameFullName)
    $protectionPayload = @{
    ...
    gh api -X PUT -H "Accept: application/vnd.github+json" "/repos/$repoNameFullName/branches/$branchName/protection" -f data=$protectionPayload
}
...
foreach ($repo in $config) {
    ...
    if ($repoUrl -match "SMA.SnowflakeTools.git") {
        ...
        # Set-BranchProtection is a function that sets branch protection (Experimental)
        #Set-BranchProtection -repoNameFullName $repoNameFullName -branchName "master"
    }
